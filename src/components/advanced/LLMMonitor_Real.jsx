...
const LLMMonitor = () => {
    ...
    const loadLLMData = async () => {
        try {
            ...
            if (response.ok) {
                ...
                console.log('✅ Loaded LLM data from server:', data.length, 'models');
            } else {
                console.error('Failed to fetch LLM data:', response.status);
                setLlmData([]);
            }
        } catch (error) {
    ...
    const updateRealTimeMetrics = async () => {
        try {
            ...
            if (response.ok) {
                const metricsData = await response.json();
                setRealTimeData(metricsData.realtime || []);

                // Update live metrics summary from real data
                if (llmData.length > 0) {
                    ...
                    const totalTokens = llmData.reduce((sum, model) => sum + (model.usage_stats?.hourly_requests || 0) * 100, 0);
                ...
                console.error('Failed to fetch real-time metrics:', response.status);
            }
        } catch (error) {
    ...
    const getPerformanceColor = (value, threshold = 95) => {
        return parseFloat(value) >= threshold ? 'text-green-400' : 'text-yellow-400';
    };
